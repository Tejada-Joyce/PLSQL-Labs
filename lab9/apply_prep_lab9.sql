...
BEGIN
  ...
  $IF $$DEBUG = 1 $THEN
     ...
     dbms_output.put_line('Primary Key: ['||primary_key_name||']');
     dbms_output.put_line('Key Value:   ['||primary_key_value||']');
     dbms_output.put_line('File Name:   ['||src_file_name||']');
  $END
...
/* Format and query columns for output. */
